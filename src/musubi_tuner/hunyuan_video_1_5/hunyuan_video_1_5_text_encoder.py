...
PROMPT_TEMPLATE_VIDEO = """<|im_start|>system
...
<|im_start|>assistant
"""
PROMPT_TEMPLATE_VIDEO_CROP_START_IDX = -1  # need to be set
...
def get_qwen_tokens(tokenizer: Qwen2Tokenizer, prompt: Union[str, list[str]] = None) -> Tuple[torch.Tensor, torch.Tensor]:
    global PROMPT_TEMPLATE_VIDEO_CROP_START_IDX

    tokenizer_max_length = TOKENIZER_MAX_LENGTH
    prompt_template_encode = PROMPT_TEMPLATE_VIDEO

    prompt = [prompt] if isinstance(prompt, str) else prompt

    if PROMPT_TEMPLATE_VIDEO_CROP_START_IDX < 0:
        # find "<|im_start|>user" token position
        prompt_example = prompt_template_encode.format("a photo of a cat")
        encoded = tokenizer(
            prompt_example, max_length=tokenizer_max_length + 256, padding=True, truncation=True, return_tensors="pt"
        )
        input_ids = encoded.input_ids[0]
        marker_tokens = tokenizer("<|im_start|>user\n", add_special_tokens=False)["input_ids"]
        drop_idx = -1
        for i in range(len(input_ids) - len(marker_tokens) + 1):
            if input_ids[i : i + len(marker_tokens)].tolist() == marker_tokens:
                drop_idx = i + len(marker_tokens)
                break
        assert drop_idx != -1, "Cannot find marker tokens in the prompt template."
        PROMPT_TEMPLATE_VIDEO_CROP_START_IDX = drop_idx
        logger.info(f"Set PROMPT_TEMPLATE_VIDEO_CROP_START_IDX to {PROMPT_TEMPLATE_VIDEO_CROP_START_IDX}")

    drop_idx = PROMPT_TEMPLATE_VIDEO_CROP_START_IDX
...
def get_qwen_prompt_embeds_from_tokens(
    ...
    tokenizer_max_length = TOKENIZER_MAX_LENGTH
    drop_idx = PROMPT_TEMPLATE_VIDEO_CROP_START_IDX
    assert drop_idx >= 0, "drop_idx should be set."
    ...
    hidden_states = encoder_hidden_states.hidden_states[-3]  # use the 3rd last layer's hidden states for HunyuanVideo 1.5
...
def get_byt5_text_tokens(tokenizer, prompt):
    ...
    try:
        ...
        # None style and color as in official implementation.
