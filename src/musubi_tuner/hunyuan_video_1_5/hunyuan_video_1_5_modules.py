...
from musubi_tuner.hunyuan_video_1_5.hunyuan_video_1_5_utils import (
    timestep_embedding,
    apply_rotary_emb,
    _to_tuple,
    apply_gate,
...
from musubi_tuner.modules.attention import attention, AttentionParams
...
class RMSNorm(nn.Module):
    ...
    def __init__(self, dim: int, eps: float = 1e-6):
        super().__init__()
        self.eps = eps
        self.weight = nn.Parameter(torch.ones(dim))  # elementwise affine parameter
    ...
    def forward(
        ...
        if self.gradient_checkpointing and self.training:
            ...
            if self.cpu_offload_checkpointing:
                forward_fn = create_cpu_offloading_wrapper(forward_fn, self.img_attn_q.weight.device)
