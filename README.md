...
This is an experimental branch for generating 1 frame videos (= images). It will be deleted soon.

This generates a video with only 1 section and 1 latent frame by specifying a large value (≒ time elapsed from the first frame) in `latent_indices`. This generates an image that changes according to the prompt from the initial image. After generation, a still image is generated by decoding the latent of the generated frame, excluding the latent of the initial image.

Specify `--video_seconds 1` to generate a single section video. The default value of `--latent_window_size` is 9, which is the RoPE value. Theoretically, specifying a large value will result in a large movement, while a small value will result in a small movement. By specifying `--output_type images`, images are output instead of .mp4.

With some trials, it seems that if the movement is large, the generation fails and does not move at all. It also seems to be strongly influenced by the random seed, so please try several times with different seeds.

# これは実験ブランチです。

1フレーム生成の実験ブランチです。近日中に消します。

1セクション、1 latent フレームのみの動画を、`latent_indices` に大きな値（≒最初のフレームから時間経過した状態）を指定して生成することで、初期画像からプロンプトに従った変化をした画像を生成します。生成後、初期画像のlatentを含まない、1 latent フレームをVAEでdecodeすることで、静止画を生成します。

`fpack_generate_video.py`を使ってください。説明は[FramePackのドキュメント](./docs/framepack.md)を参照してください。

1セクションのみの生成とするため、`--video_seconds`に`--video_seconds 1`のように小さい値を指定してください。`--latent_window_size`はデフォルト9ですが、この値がRoPEの値になるので、理論的には大きな値を指定すると動きが大きく、小さな値だと小さくなるはずです。`--output_type images` を指定することで .mp4 ではなく、画像を出力します。

試したところ、動きが大きい場合には生成が失敗し、微動だにしません。また乱数seedの影響を強く受けるようですので、seedを変えて何回か試してください。
